...
//    InitLLVM X(argc, argv);
    ...
    if (!linkFiles(exe, Context, L, InputFilenames, Flags))
        return 1;

    // Next the -override ones.
    if (!linkFiles(exe, Context, L, OverridingInputs,
        Flags | Linker::Flags::OverrideFromSrc))
        return 1;

    // Import any functions requested via -import
    if (!importFunctions(exe, *Composite))
        return 1;
    ...
    std::error_code EC;
    ToolOutputFile Out(StringRef(output), EC, sys::fs::F_None);
    if (EC) {
